...
class MarketingContentViewSet(viewsets.ModelViewSet):
    ...
    def trigger_agent(self, request, pk=None):
        ...
        if agent_response.status == "completed" and agent_response.artifacts:
            ...
            # Görsel Oluşturucu seçiliyse ayrıca görsel de üret
            if content.content_type == "IMAGE":
                image_bytes, image_error = client.generate_image(
                    brand_name=content.campaign.brand.name,
                    vertical=content.campaign.brand.vertical,
                    platform=content.platform,
                    tone=content.tone or "professional",
                    topic=content.topic or generated.title,
                )
                if image_bytes:
                    dosya_adi = f"content_{content.id}_{uuid.uuid4().hex[:8]}.png"
                    kayit_yolu = default_storage.save(
                        f"generated_images/{dosya_adi}",
                        ContentFile(image_bytes)
                    )
                    content.media_url = request.build_absolute_uri(
                        f"{settings.MEDIA_URL}{kayit_yolu}"
                    )
                else:
                    logger.error(f"Görsel üretilemedi: {image_error}")
        else:
            content.status = "FAILED"
            hashtag_listesi = []
